...
public class OracleGenerator implements Generator {
	private static String tag = "oracle"; 
	private Configuration cfg;
	private OracleGenerator() {
		cfg = new Configuration();  
        // 指定FreeMarker模板文件的位置  
        try {
			cfg.setDirectoryForTemplateLoading(new File(OracleGenerator.class.getProtectionDomain().getCodeSource().getLocation().getPath()+"templates"));
		} catch (IOException e) {
			e.printStackTrace();
		}
        
	}
	private static final OracleGenerator instance = new OracleGenerator();
	public static OracleGenerator getInstance(){
		return instance;
	}
	@Override
	...
	public void createAllFile(BaseParam param, List<TableField> fieldlist){
		for(TableField f:fieldlist){
			if(f.getColumnKey()!=null){
				param.setPrimaryKey(f.getColumnName().toLowerCase());
	...
	private String getJavaType(String type){
		...
		if(type.toLowerCase().indexOf("number")>=0){
			if(type.indexOf("(")>=0){
				return "Float";
			}else{
				return "Integer";
			}
		}else if(type.toLowerCase().indexOf("varchar2")>=0 || type.toLowerCase().indexOf("nclob")>=0 || type.toLowerCase().indexOf("char")>=0){
			return "String";
	...
	public String getJdbcType(String type){
		...
		}else if(type.toLowerCase().indexOf("varchar2")>=0){
			return "VARCHAR";
		}else if(type.toLowerCase().indexOf("char")>=0){
			return "CHAR";
		}else if(type.toLowerCase().indexOf("timestamp")>=0 || type.toLowerCase().indexOf("date")>=0){
			return "TIMESTAMP";
		}else if(type.toLowerCase().indexOf("nclob")>=0){
			return "NCLOB";
		}
		return "";
	}
